...
class AddMyMovie extends Component {
    ...
    selectMovie = (result) => {
        ...
        this.props.dispatch({ type: 'ADD_USER_MOVIE', payload: { movieToAdd: result, user_id: this.props.reduxState.user.id} })
    }

    addMovie = () => {
        this.props.dispatch({ type: 'FETCH_MOVIE_ID', payload: { queryText: this.state.movieToAdd.original_title, user_id: this.props.reduxState.user.id} })
    ...
    render() {
        ...
        if (this.state.showSuggestions){
            suggestions = <>
                <h4 className="header-4">Did you mean:</h4>
       ...
       {movieDetails}
            ...
            <div className="movie-buttons">
                <div className="button-wrapper">
                    <button className="addMovieBtn" onClick={(event) => this.addMovie()}>Add Movie</button>
                    </div>
                    <div className="button-wrapper">
                    <Link to="/MyProfile" className="addMovieBtn">Cancel</Link>
                    </div>
            </div>
    </>
)
